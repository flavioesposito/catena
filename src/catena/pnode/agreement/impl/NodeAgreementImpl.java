 ...
 */
package catena.pnode.agreement.impl;
...
import catena.impl.googleprotobuf.CAD;
import catena.impl.googleprotobuf.CAD.CADMessage.assignment;
import catena.impl.googleprotobuf.CAD.CADMessage.bid;
import catena.impl.googleprotobuf.CAD.CADMessage.bidTime;
import catena.pnode.api.AgreementAPI;
...
public class NodeAgreementImpl implements AgreementAPI {
	...
	private votingData _newvotingData = null;
	/**
	 ...
	 */
	public votingData nodeAgreement(CAD.CADMessage cadMessage, votingData _currentvotingData, 
			String k,   //sender pnode 
			String i) { //receiver pnode

		
		_newvotingData = _currentvotingData;

		// assume there is no need to rebroadcast
		_newvotingData.set_rebroadcast(false);
		...
		rib.RIBlog.debugLog("NodeAgreement::nodeAgreement: _currentvotingData : "+_currentvotingData);

		rib.RIBlog.debugLog("NodeAgreement::nodeAgreement: get_allocationVectorMap() : "+_currentvotingData.get_allocationVectorMap());

		if(_currentvotingData.get_allocationVector(received_sliceID) == null){
			...
			a_i = _currentvotingData.get_allocationVector(received_sliceID);
		}
		...
		if(_currentvotingData.get_bidVector(received_sliceID) == null){
			...
			b_i = _currentvotingData.get_bidVector(received_sliceID);
		}
		...
		if(_currentvotingData.get_votingTimeVector(received_sliceID) ==null )
		{
		...
		t_i = _currentvotingData.get_votingTimeVector(received_sliceID);
		...
		if( _currentvotingData.get_mMap().get(received_sliceID) ==null )
		{
		...
		m_i = _currentvotingData.get_mMap().get(received_sliceID); 
		...
		for(int j = 0; j< b_k.size(); j++)
		{
			...
			//TODO: Improvement: as soon as we realize we need to release a node in the bundle:
					...
					// redo the voting phase taking into account the lost bid (otherwise bids are too conservative)
		...
		updateVectors(_currentvotingData, cadMessage.getSliceID());

		
		return _newvotingData;
	}
	 ...
	 * @param _currentvotingData
	 */
	public void updateVectors(votingData _currentvotingData, int sliceID) {


		_newvotingData.set_votingTimeVector(t_i, sliceID);

		LinkedHashMap<Integer, LinkedHashMap<Integer,Long>> _votingTimeMap = _currentvotingData.get_votingTimeMap();
		_votingTimeMap.remove(sliceID);
		_votingTimeMap.put(sliceID, t_i);
		_newvotingData.set_votingTimeMap(_votingTimeMap);

		_newvotingData.set_allocationVector(sliceID, a_i);

		LinkedHashMap<Integer, LinkedHashMap<Integer,String>> _allocationVectorMap = _currentvotingData.get_allocationVectorMap();
		_allocationVectorMap.remove(sliceID);
		_allocationVectorMap.put(sliceID, a_i);		 
		_newvotingData.set_allocationVectorMap(_allocationVectorMap);

		_newvotingData.set_bidVector(sliceID, b_i);

		LinkedHashMap<Integer, LinkedList<Double>> _bidVectorMap = _currentvotingData.get_bidVectorMap();
		_bidVectorMap.remove(sliceID);
		_bidVectorMap.put(sliceID, b_i);
		_newvotingData.set_bidVectorMap(_bidVectorMap);


		_newvotingData.set_m(sliceID, m_i);

		LinkedHashMap<Integer, LinkedList<Integer>> _mMap = _currentvotingData.get_mMap();
		_mMap.remove(sliceID);
		_mMap.put(sliceID, m_i);
		_newvotingData.set_mMap(_mMap);

		_newvotingData.setA_i(a_i);
		_newvotingData.setA_k(a_k);
	...
	public void rebroadcast() {
		...
		this._newvotingData.set_rebroadcast(true);
	}
	...
	public void releaseSubsequentVnodes(int j) {
		//if the node was in the bundle you need to release the subsequent and rebid forvoting the node just lost
